    ...
    * {box-sizing: border-box}
    ...
    function validate_encryption() {
        ...
        if (!reg_marker.test(plain.value) && !reg_plain.test(plain.value)) {
            window.alert("Only spaces, alphabets and digits are allow for A5/1 cipher encryption");
            return false;
        }
        return true;
    ...
    function validate_decryption() {
        ...
        if (!reg_marker.test(cipher.value) && !reg_cipher.test(cipher.value)) {
            window.alert("0 and 1 only for A5/1 cipher decryption");
            return false;
        }
        return true;
        ...
        if( preg_match("/^\*$/", $_POST['etext']) or preg_match("/^[a-zA-Z0-9 ]+$/", $_POST['etext'])) {
            ...
            if ($_FILES and $_POST['etext'] == "*") {
                ...
                if(!preg_match("/^[a-zA-Z0-9 ]+$/", file_get_contents($f_name)))
                    die("<script type='text/javascript'>alert('Only spaces, alphabets and digits are allow for A5/1 cipher encryption');</script>");
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if( preg_match("/^\*$/", $_POST['dtext']) or preg_match("/^[10]+$/", $_POST['dtext'])) {
            ...
            if ($_FILES and $_POST['dtext'] == "*") {
                ...
                //check upload file
                if(!preg_match("/^[10]+$/", file_get_contents($f_name)))
                    die("<script type='text/javascript'>alert('0 and 1 only for A5/1 cipher decryption');</script>");
